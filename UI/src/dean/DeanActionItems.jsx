...
export default class DeanActionItem extends Component {
    ...
    constructor(props) {
        ...
        this.onDeanFileChange = (e) => {
            ...
            this.setState({ deanFormData: this.encodeDeanFileBase64(e.target.files) })
    ...
    renderTableHeader() {
        let header = ["Application Id", "Student Id", "Name", "Status", "Employer Form", "Advisor Form", "Upload Signed Advisor Form", "Action", "Submit"]
    ...
    renderTableData() {
        return (this.state.applicationData).map((d, index) => {
            const { a_id, uid, current_status, student_name, next_approver, advisor_form, employer_form } = d //destructuring
            let role = ''
            if (next_approver === 1) {
            ...
            return (
                <>
                    <tr key={index}>
                        ...
                        <td>
                            <Base64Downloader base64={advisor_form} downloadName="pdfDownload_adv">
                                Click to download Advisor Form
                            </Base64Downloader>
                        </td>
                        <td>
                            <Base64Downloader base64={employer_form} downloadName="pdfDownload_emp">
                                Click to download Employer Form
                            </Base64Downloader>
                        </td>
                        <td>
                            <input class="form-control form-control-sm" id="EmpForm" type="file" name="file" required onChange={this.onDeanFileChange} />
                        </td>
                        <td>
                            <div className="form-check form-check-inline" align='right'>
                                <input type="radio" value="Approved" name="action" checked={this.state.selectedOption === "Approved"} onChange={this.onValueChange} />Approve
                            </div>
                            <div className="form-check form-check-inline">
                                <input type="radio" value="Rejected" name="action" checked={this.state.selectedOption === "Rejected"} onChange={this.onValueChange} />Reject
                            </div>
                        </td>
                        <td>
                            <button
                                ...
                                onClick={
                                    async (e) => {
                                        ...
                                        console.log("role_id:: " + parseInt(window.localStorage.getItem("role_id")))
